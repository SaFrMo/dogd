...
public class Terminal5_sc2 : Conversation {
	...
	protected override void GetContent (int key, out string content, out Dictionary<string, int> playerLines) {
		...
		case 0:
			...
			playerLines = new Dictionary<string, int>() {
				{ "Double-checking this terminal - is there any unique data?", 1 }
			};
			break;

		case 1:
			toContent = (player.GetComponent<Inventory>().inventory.Contains ("Terminal5_sc2 Data") ? 
			             "None." :
			             "Yes");
			break;
