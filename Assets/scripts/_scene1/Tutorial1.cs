...
public class Tutorial1 : Conversation {
	...
	protected override void GetContent (int key, out string content, out Dictionary<string, int> playerLines) {
		...
		case 0:
			toContent = "You carry zero-G manuevering fuel, but only for emergencies. You use your magnetic boots by default," +
				" which require an orientation point.";
			AllowContinue();
			break;

		case 1:
			toContent = "Click any object that highlights green to orient yourself to that point.";
			AllowContinue();
			break;
